...
function handleFiles() {
    ...
    var SHA256 = CryptoJS.algo.SHA256.create();//   SHA256 Algoritmasına göre 
    var counter = 0;
    var self = this;

    var timeStart = new Date().getTime();
    var timeEnd = 0;
    $("#timeStart").val(new Date(timeStart)); // Time Start 
    ...
    loading(file,
        function (data) {
            ...
            $("#hash").val(encrypted);
            timeEnd = new Date().getTime();// TimeEnd
            $("#timeStart").val(new Date(timeStart));// Start Time  
            $("#timeEnd").val(new Date(timeEnd));    // End Time
            $("#timeDelta").val((timeEnd - timeStart) / 1000 + ' sec'); //Response Time-krc
